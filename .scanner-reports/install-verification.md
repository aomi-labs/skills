# Install verification — `aomi-transact`
...
Cannot fully exercise this without a Claude Code instance on the verifying machine, but the JSON validates against the same schema used by Anthropic's `claude-plugins-official` marketplace and the cryptoskills marketplace.
