...
- **`aomi deploy` is advertised but not wired up.** It appears in `aomi --help` under COMMANDS, but it is not a registered subcommand in v0.4.2 — invoking it drops into the interactive REPL instead of deploying. Do not use it.
