...
- **"`aomi wallet set` is EVM-only"** → Current CLI supports `aomi wallet set --solana <base58-or-json-keypair>` and `--solana-private-key` / `SOLANA_PRIVATE_KEY` for Solana sign-only flows. The CLI still only materializes pending Solana transactions that include an unsigned transaction payload; instruction-only `svm_ixs` requests may require product work before the npm signer can handle them.
