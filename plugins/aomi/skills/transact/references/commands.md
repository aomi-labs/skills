...
aomi wallet set <signing-key>                          # user-directed EVM 0x key
aomi wallet set --solana <base58-or-json-keypair>       # user-directed Solana keypair
aomi wallet login                                      # browser auth URL, mint account bearer
aomi wallet whoami                                     # inspect authenticated account
aomi account login                                     # alias for account auth
aomi account whoami
aomi config current                                    # backend URL
aomi config set-backend <url>                          # repoint CLI at a different backend
```

`aomi wallet set` persists signing material under `AOMI_STATE_DIR`. After running, confirm with the derived address or Solana public key — never repeat the key value back.
