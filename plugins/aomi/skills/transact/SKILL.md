...
- **Per-wallet signing modes.** Every linked wallet carries a signing policy — 🟢 `autonomous`, 🟠 `human_sync`, 🔴 `denied` — inspected via `aomi wallet ls` and changed only through the signed EIP-712 permit ceremony of `aomi wallet set-mode` (challenge → sign → commit). Autonomous signing additionally requires a live delegated grant; if it is missing or expired, run `aomi login --provider privy` first.
...
aomi login|logout                   BetterAuth device flow; --provider privy connects/disconnects a wallet provider
